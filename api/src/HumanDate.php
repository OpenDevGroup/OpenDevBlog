...
class HumanDate {
     ...
     */
    private static function humanFormat($difference, $unit) {
    ...
    /**
     * Returns a human readable string corresponding to the supplied
    ...
    public static function parse(\DateTime $dateTime, \DateTime $reference = null) {
        ...
        if ($reference->format('Y/m/d') == $date) {
            if (0 <= $difference && $absDiff < self::MINUTE) {
                ...
                return self::humanFormat($difference / self::MINUTE, 'minute');
            } else {
                return self::humanFormat($difference / self::HOUR, 'hour');
            }
        }
        ...
        if ($yesterday->format('Y/m/d') == $date) {
            ...
            return self::humanFormat($difference / self::DAY, 'day');
        } else if ($absDiff / self::WEEK <= 5) {
            return self::humanFormat($difference / self::WEEK, 'week');
        } else if ($absDiff / self::MONTH < 12) {
            return self::humanFormat($difference / self::MONTH, 'month');
        }
        
        // Over a year ago
        return self::humanFormat($difference / self::YEAR, 'year');
    }
}